__author__ = 'Copyright (c) 2017 Francisca Martinez All rights reserved.'
"""
PyTimingTask:
Package implementing the two-interval timing task in mice, using Arduino
Fri Apr 07 2017 by Minggui Chen and Francisca Martinez
...
animalTag = 'm212_121121_33'                                           #set the filename of your data here
...
rewardDuration = 320                                          #how long you want to deliver reward
vacuumDuration = 100
airpuffDuration = 200                                            # airpuff punishment
...
rewardPin_1 = 44                                                #pin used to control the gate for the reward
rewardPin_2 = 48
vacuumPin = 46
airpuffPin = 42
BOARD_LED = 10
...
def EnableSpout(status):
    ...
    if (status):  #spout on, servo to degs
        board.analog_write(servoPin, 100)
    else:       #spout off, servo to degs
        board.analog_write(servoPin, 20)
...
def PlayLED(time_led):
    global BOARD_LED
    for i in range(time_led):
...
def LickometerCallback_1(data_1):
    ...
    if enableTrial and not thisResponseType:
        if (enableLick_1 and len(bLicked_1)>2):                                            #lick correctly
            thisResponseType = response.CORRECT
            Reward(rewardDuration, reward)
            thistrialStamp.append(clock)
            thisLickClock_1.append(clock)
            print('touch 1')
        elif (breakLickEarly and clock>lickWinClock and not enableLick_1 and len(bLicked_1) >3):                   #lick early
    ...
    if Passive == True:
        thisResponseType = response.PASSIVE
...
def LickometerCallback_2(data_2):
    ...
    if enableTrial and not thisResponseType:
        if (enableLick_2 and len(bLicked_2)>2):                                            #lick correctly
            thisResponseType = response.CORRECT
            Reward(rewardDuration, reward)
            thisLickClock_2.append(clock)
            thistrialStamp.append(clock)
            print('touch 2')
        elif (breakLickEarly and clock>lickWinClock and not enableLick_2 and len(bLicked_2) >3):                   #lick early
            thisResponseType = response.INCORRECT
            Airpuff(airpuffDuration)
            thisLickClock_2.append(clock)
            thistrialStamp.append(clock)
...
def sensory_button():
    global iti, rewardPin_1, rewardPin_2, Normal, Forced_Right, Forced_Left, servoTime, n_intervals, Passive
    print("start")
    def print_value():
        global iti
        value = delay.get()
    ...
    def f_Normal():
        ...
        currect_strategy.place(x=150, y=330)
    ...
    def f_Passive():
        global Passive
        if Passive_buttom['background'] == 'red':
            Passive_buttom.configure(background='gold')
            Passive = True
            print('Passive Mode = ON')
        else:
            Passive_buttom.configure(background='red')
            Passive = False
    ...
    selected_strategy = tk.Label(root, text='Selected Strategy :', font='Helvetica 9 bold')
    selected_strategy.place(x=10, y=330)

    Passive_buttom = tk.Button(root, text='PASSIVE', width=12, background='red', font= 'Helvetica 10 bold', command=f_Passive)
    Passive_buttom.place(x=115, y=360)
    root.mainloop()
...
def GenerateCondition():
    global initial_sequence, responseType, Normal, Forced_Right, Forced_Left
...
while numValidTrials < numTrials:
    ...
    #generate this stimulus condition return index number
        timingID = int(GenerateCondition())
    else:
    ...
    thistrialStamp.append(clock)
    thisToneStamp.append(clock)
    # second_tone = thisToneStamp[-1] + thisInterval

    #spout_presentation = thisToneStamp[-1] + servoTime + toneDuration + 100
    ...
    if Passive == False:
        ...
        thistrialStamp.append(np.nan)
        thistrialStamp.append(np.nan)
        ax1.plot(i+1, resp, 'o', markersize=10, fillstyle='none', markeredgecolor='grey', markeredgewidth=1.5)
    ...
    tonesStamp = [thisToneStamp[1], thisToneStamp[2]]
    led_time = thisToneStamp[0]
